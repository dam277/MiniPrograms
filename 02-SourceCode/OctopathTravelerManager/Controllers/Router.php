...
class Router 
{
     ...
     * @return Response|null The response object or null if no route matches.
     */
    public static function route(string $method, ?string $action): Response|null 
    {
        ...
        [$route, $params] = self::getRoute($method, $action);
        
        // If no route is found, return a 404 response
        if ($route == null)
            return new Response(httpCode: 404, responseString: View::get("errors.default", [
                "errorCode" => 404,
                "errorMessage" => "Not Found",
                "errorDescription" => "The requested action '$action' was not found."
            ]));
        ...
        if (!class_exists($route?->getController()) || !method_exists($controller, $function))
            return new Response(httpCode: 500, responseString: View::get("errors.default", [
                "errorCode" => 500,
                "errorMessage" => "Internal Server Error",
                "errorDescription" => "Controller {$route?->getController()} or function $function not found."
            ]));
